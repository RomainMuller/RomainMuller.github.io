...
const INTEGRITY_HASHES = ['sha512', 'sha384', 'sha256'];
...
module.exports = function (/** @type import('@11ty/eleventy').UserConfig */ eleventyConfig) {
  ...
  eleventyConfig.addAsyncShortcode('link', async (rel, href) => {
    ...
    const integrity = await Promise.all(
      INTEGRITY_HASHES.map(async (hashfn) => {
        const asset = path.join(eleventyConfig.dir.output, href);
    ...
    return `<link rel="${rel}" href="${href}" integrity="${integrity.join(' ')}">`
  });
  eleventyConfig.addAsyncShortcode('script', async (src) => {
    ...
    const integrity = await Promise.all(
      INTEGRITY_HASHES.map(async (hashfn) => {
        const asset = path.join(eleventyConfig.dir.output, src);
        const content = await retry(fs.readFile)(asset);
        const hash = crypto.createHash(hashfn)
          .update(content)
          .digest('base64');
        return `${hashfn}-${hash}`;
    ...
    return `<script src="${src}" integrity="${integrity.join(' ')}"></script>`
  });
